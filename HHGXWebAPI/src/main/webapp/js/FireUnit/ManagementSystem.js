(function () {
    ...
    function pageReload(nowNum) {
        ...
        HH.post("/ManageRule/SafeManageRulesList", info, function (data) {
    ...
    getCheck("/Common/GetDictionary", {TypeName: "制度类型"}, "#check_add");
    ...
    $("#add_btn").click(function () {
        ...
        if (add_state) {
            ...
            var options = {
                url: ApiUrl + "/Form/AddSafeManageRules", //默认是form的action， 如果申明，则会覆盖
    ...
    $("#in_page").on("click", ".btn_edit", function () {
        ...
        HH.post("/ManageRule/GetSafeManageRules", {SafeManageRulesID: operationId}, function (data) {

            var myJson = data.DataBag;

            render("#editView", "#editPage", myJson);

            $("#SafeManageRulesID").val(operationId);
            //上传文件按钮
            ...
            var typeArr = myJson.SafeManageRulesType.split(",");
            getCheck("/Common/GetDictionary", {TypeName: "制度类型"}, "#check_edit", typeArr);
            ...
            $("#edit_btn").click(function () {
                ...
                if (add_state) {
                    ...
                    var options = {
                        url: ApiUrl + "/Form/UpdateSafeManageRules", //默认是form的action， 如果申明，则会覆盖
                        type: "post",               //默认是form的method（get or post），如果申明，则会覆盖
                        dataType: "json",           //html(默认), xml, script, json...接受服务端返回的类型
                        //clearForm: true,          //成功提交后，清除所有表单元素的值
                        //resetForm: true,          //成功提交后，重置所有表单元素的值
    ...
    $("#deleteModal").on("click", "#deleteModal_btn", function () {
        HH.post("/ManageRule/DeleteSafeManageRules", {SafeManageRulesID: deleteId}, function (data) {
