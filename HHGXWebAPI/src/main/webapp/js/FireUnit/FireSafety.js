(function () {
    ...
    function pageReload(nowNum) {
        ...
        HH.post("/Facility/getTrainingList", info, function (data) {
    ...
    $("#add_btn").click(function () {
        ...
        var addList = [
            "#form_add input[name='TrainingTime']",
            "#form_add input[name='TrainingAddress']",
            "#form_add input[name='HowmanyPeople']",
            "#form_add textarea[name='TrainingContent']"
        ];
        ...
        if(addState){
            ...
            var options = {
                url: ApiUrl + "/Form/AddTraining", //默认是form的action， 如果申明，则会覆盖
    ...
    $("#in_page").on("click", ".btn_check", function () {
        ...
        HH.post("/Facility/GetTraingingDetail", {TrainingID: operationId}, function (data) {
            var myJson = data.DataBag[0];

            if (myJson.ContentFile) {
                myJson["ContentFileName"] = myJson.ContentFile.split("/").pop();
            }
            if (myJson.examfile) {
                myJson["examfileName"] = myJson.examfile.split("/").pop();
            }
            if (myJson.signtable) {
                myJson["signtableName"] = myJson.signtable.split("/").pop();
            }
    ...
    $("#in_page").on("click", ".btn_edit", function () {
        ...
        HH.post("/Facility/GetTraingingDetail", {TrainingID: operationId}, function (data) {
            ...
            if (myJson.ContentFile) {
                myJson.ContentFile = myJson.ContentFile.split("/").pop();
            }
            if (myJson.examfile) {
                myJson.examfile = myJson.examfile.split("/").pop();
            }
            if (myJson.signtable) {
                myJson.signtable = myJson.signtable.split("/").pop();
            }

            render("#editView", "#editPage", myJson);
            dataPick("#fireDate_edit");
            $("#TrainingType_edit").val(myJson.TrainingType);
            ...
            $("#edit_btn").click(function () {
                ...
                var editList = [
                    "#form_edit input[name='TrainingTime']",
                    "#form_edit input[name='TrainingAddress']",
                    "#form_edit input[name='HowmanyPeople']",
                    "#form_edit textarea[name='TrainingContent']"
                ];
                ...
                if(editState){
                    var options = {
                        url: ApiUrl + "/Form/UpdateTraining", //默认是form的action， 如果申明，则会覆盖
                        type: "post",               //默认是form的method（get or post），如果申明，则会覆盖
                        dataType: "json",           //html(默认), xml, script, json...接受服务端返回的类型
                        //clearForm: true,          //成功提交后，清除所有表单元素的值
                        //resetForm: true,          //成功提交后，重置所有表单元素的值
    ...
    $("#deleteModal").on("click", "#deleteModal_btn", function () {
        HH.post("/Facility/DeleteTraining", {TrainingID: deleteId}, function (data) {
