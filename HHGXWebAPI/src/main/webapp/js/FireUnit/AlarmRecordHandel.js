 ...
 */
(function(){
    //console.log($.trim(sessionStorage.getItem("Firealarmid")));
    HH.post("/AlarmData/FireDealInfo",{"Firealarmid": $.trim(sessionStorage.getItem("Firealarmid"))},function(data) {
    ...
    $(document.body).on("click","#navBar>li",function(){
        if($(this).text()=="地图"){
            ...
            //console.log(temp);
            if(temp.attr("id")!=undefined && myAllData.DataBag.length>0){
                //baiDuMap(temp.attr("id"),"lng","lat");
                //searchMap("lng","lat",temp.attr("id"));
        ...
        if($(this).text()=="处理详情"){
            render("#handelDetailData","#targBaseInfoRow",myAllData);
            $("#navBar>li").css("border-bottom","none");
            $("#navBar>li:nth-of-type(6)").css("border-bottom","3px solid gray");
